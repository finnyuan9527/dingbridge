...
这些事件在落库的同时仍保留标准 logger 输出。审计日志不会记录 token、client secret 或钉钉 app secret，只保存事件类型、client、IP、用户基础标识和必要上下文；日志保留周期需要由部署方在数据库侧自行治理。

当前版本还没有提供审计日志查询页面或查询 API，如需排查，可直接查询数据库，例如：
...
```env
DINGBRIDGE_IMAGE=ghcr.io/finnyuan9527/dingbridge:v2.0.0
```
...
注意：`.env.example` 的默认 `DATABASE__URL=sqlite:////data/dingbridge.sqlite3` 是 Docker 部署路径；源码启动时请改成上面的本地路径，或改成你的 MySQL DSN。
...
cd dingbridge
cp .env.example .env
# Edit .env. For minimal local validation, make sure:
# - SECURITY__ALLOW_EPHEMERAL_KEYS=true
# - REDIS__PASSWORD=dingbridge_redis_password
# - DINGBRIDGE_IMAGE=ghcr.io/finnyuan9527/dingbridge:latest
...
These events are still emitted to the normal logger as well. Audit logs do not store tokens, client secrets, or DingTalk app secrets; they only keep the event type, client, IP, basic user identifiers, and required context. Retention should be managed by the deployment owner at the database level.

There is no audit log UI or query API yet, so for now you should inspect the database directly, for example:
...
Note: `.env.example` defaults to `DATABASE__URL=sqlite:////data/dingbridge.sqlite3` for Docker deployments. When running from source, change it to the local SQLite path shown above, or to your MySQL DSN.
